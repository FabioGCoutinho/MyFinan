...
export default function NovaDespesaPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!expense.trim() || !value || !date || !category) {
      setError('Preencha os campos obrigatórios (Despesa, Valor, Data, Categoria).')
      return
    }
    ...
    if (isInstallment) {
      try {
        ...
        setError(error instanceof Error ? error.message : 'Erro ao cadastrar despesa')
        setIsDisabled(false)
        return
      }
    ...
    <>
      <div className="container mx-auto py-8 px-4 max-w-5xl">
        ...
        <nav className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
          ...
          <Link href="/despesas" className="hover:text-foreground transition-colors font-semibold">
            Despesas
          </Link>
          <span className="text-muted-foreground/50">›</span>
          <span className="text-foreground font-semibold">Nova Despesa</span>
        </nav>
          ...
          <div className="lg:col-span-7 bg-[#f8fafc] dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[32px] p-6 sm:p-10 shadow-sm relative overflow-hidden">
            ...
            <form onSubmit={handleSubmit} id="expense-form" className="space-y-5 relative z-10">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="expense" className="text-xs font-bold text-slate-700 dark:text-slate-300 ml-1">
                  Descrição do Gasto
                </Label>
                <Input
                  id="expense"
             ...
             <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[32px] p-6 shadow-xl shadow-slate-200/50 dark:shadow-none">
                 ...
                 <div className="mb-6 flex flex-col items-center justify-center">
                     ...
                     <div className="flex items-center gap-1.5 text-emerald-600 dark:text-emerald-400">
                        ...
                        <span className="text-[11px] font-bold text-center">
                          Lançamento em {date ? format(new Date(date + "T00:00:00"), 'MMMM', { locale: ptBR }) : 'sua conta'}
                        </span>
                     </div>
                 </div>
                 ...
                 <div className="bg-[#eff2fc] dark:bg-slate-800/60 rounded-[18px] p-4 flex gap-3 items-start">
                    ...
                    <p className="text-[11px] font-semibold text-slate-600 dark:text-slate-400 leading-relaxed">
                      Esta transação atualizará o seu saldo disponível imediatamente e será refletida nos gráficos mensais.
                    </p>
                 </div>
             </div>
        ...
        <AlertDialogContent className="max-w-sm text-center">
          <AlertDialogHeader className="flex flex-col items-center gap-3">
            ...
            <AlertDialogDescription className="text-sm text-muted-foreground font-medium">
              Sua transação foi registrada com sucesso e já está impactando seu fluxo de caixa mensal.
            </AlertDialogDescription>
          </AlertDialogHeader>
